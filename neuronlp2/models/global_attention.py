...
class GlobalAttention(nn.Module):
    ...
    * Bahdanau Attention (mlp):
    ...
    def score(self, h_t, h_s):
        ...
        if self.attn_type in ["general", "dot"]:
            ...
            if self.attn_type == "mlp":
                ...
                return self.v(wquh.view(-1, dim)).view(tgt_batch, tgt_len, src_len, dim)
    ...
    def forward(self, input, memory_bank, memory_lengths=None, coverage=None):
        ...
        # avoid NaN
        length_eq_1 = [i for i, it in enumerate(memory_lengths.tolist()) if it == 1]
        length_eq_1_cuda = torch.LongTensor(length_eq_1).cuda()
        if len(length_eq_1) > 0:
            mask[:,0,0].index_fill_(0, length_eq_1_cuda, 1)
        ...
        if len(length_eq_1) > 0:
            align_vectors[length_eq_1].fill_(0)
        ...
        if one_step:
            ...
            aeq(batch, batch_)
            aeq(dim, dim_)
            _s = align_vectors.size()
            batch_, sourceL_ = _s[0], _s[1]
            aeq(batch, batch_)
            aeq(sourceL, sourceL_)
        else:
            ...
            aeq(dim, dim_)
            _s = align_vectors.size()
            targetL_, batch_, sourceL_ = _s[0], _s[1], _s[2]
            aeq(targetL, targetL_)
            aeq(batch, batch_)
            aeq(sourceL, sourceL_)
