...
def load_embedding_dict(embedding, embedding_path, normalize_digits=True):
    ...
    if embedding == 'word2vec':
        ...
        word2vec = Word2Vec.KeyedVectors.load_word2vec_format(embedding_path, binary=True)
        embedd_dim = word2vec.vector_size
        return word2vec, embedd_dim
    elif embedding == 'glove' or embedding == 'elmo':
        # loading GloVe
        embedd_dim = -1
        embedd_dict = dict()
        ...
        raise ValueError("embedding should choose from [word2vec, senna, glove, sskip, polyglot, elmo]")
