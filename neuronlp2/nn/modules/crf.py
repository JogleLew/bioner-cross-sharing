...
from neuronlp2.nn import utils
...
class ChainCRF(nn.Module):
    ...
    def loss(self, input, target, mask=None, reflect=None):
        '''
    ...
    def decode(self, input, mask=None, reflect=None, leading_symbolic=0):
        """
...
class NoCRF(nn.Module):
    def __init__(self, input_size, num_labels, **kwargs):
        super(NoCRF, self).__init__()
        self.input_size = input_size
        self.num_labels = num_labels + 1
        self.pad_label_id = num_labels
        self.trans = nn.Linear(input_size, self.num_labels)
    ...
    def loss(self, input, target, mask=None, reflect=None):
        ...
        predict = self.softmax_dim2(self.trans(input))
        batch, length = target.size()
        if mask is not None:
            _, length = mask.size()
            target = target[:,:length].contiguous()
        # [batch * length, 1]
        target = target.view(batch * length, 1)
        # [batch, length, num_labels]
        target = Variable(
            utils.onehot(target, self.num_labels).view(batch, length, -1)
        )
